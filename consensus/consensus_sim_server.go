...
import (
	...
	"github.com/iotexproject/iotex-core-internal/consensus"
...
func (s *server) Init(in *pb.InitRequest, stream pb.Simulator_InitServer) error {
	...
	for i := 0; i < int(nPlayers); i++ {
		...
		rawAddrs = append(rawAddrs, addr.RawAddress)
	}

	fmt.Println(addrs)
	fmt.Println(rawAddrs)
	...
	for i := 0; i < int(nPlayers); i++ {
		...
		cfg, err := config.New()
		if err != nil {
			logger.Error().Err(err).Msg("could not find config file")
		}

		// handle node address, delegate addresses, etc.
		cfg.Delegate.Addrs = rawAddrs
		cfg.Network.Addr = "127.0.0.1:10000"
		cfg.Network.NumPeersLowerBound = 6
		cfg.Network.NumPeersUpperBound = 12

		cfg.Chain.ProducerPrivKey = hex.EncodeToString(addrs[i].PrivateKey)
		cfg.Chain.ProducerPubKey = hex.EncodeToString(addrs[i].PublicKey)
	...
	time.Sleep(5*time.Second) // TODO: do a waitUntil instead of sleeping
